...
ON CONFLICT (code) DO UPDATE SET display_name=EXCLUDED.display_name,model_id=EXCLUDED.model_id,status=EXCLUDED.status,updated_at=now();

ALTER TABLE system_ai_settings
  ADD COLUMN IF NOT EXISTS selected_capability_code TEXT REFERENCES ai_capabilities(code),
  ADD COLUMN IF NOT EXISTS classnote_model_code TEXT REFERENCES ai_model_engines(code),
  ADD COLUMN IF NOT EXISTS primary_engine_code TEXT REFERENCES ai_model_engines(code);

UPDATE system_ai_settings SET
 selected_capability_code=COALESCE(selected_capability_code,'knowledge.answer'),
 classnote_model_code=COALESCE(classnote_model_code,'classnote-knowledge-g2'),
 primary_engine_code=COALESCE(primary_engine_code,'engine-classnote')
WHERE id=1;

CREATE INDEX IF NOT EXISTS idx_ai_model_engines_role_status ON ai_model_engines(engine_role,status);
